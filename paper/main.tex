...
free-form recaps are \emph{verbose but imprecise} -- DeepSeek-V3.2 reaches recall
$\RSDeepseekRecall{}$ at precision only $\RSDeepseekPrec{}$ -- while the terse model stays
precise but partial (gemini-2.5-pro $\RSGeminiPrec{}$/$\RSGeminiRecall{}$). Ranking by
precision and by $F_1$ again disagree. Precision and recall capture both failure modes;
neither alone does.
...
A natural objection is that low coverage is an artifact of under-prompting: the default
prompt even asks the model to be \emph{concise}. We test this directly with a prompt
ablation -- the same data and metric, but an explicit \emph{cover-all} prompt that asks
the model to state every supportable fact (pit laps, compounds, stops, the move and its
outcome, time gained, positions). Table~\ref{tab:ablation} compares the two. Coverage is
indeed prompt-sensitive: $\AblNUp{}$ of $\AblNModels{}$ models raise recall when asked to
be thorough (mean recall $\AblMeanRecallA{}\!\to\!\AblMeanRecallB{}$). But this
\emph{strengthens}, not weakens, the case for measuring coverage. (i)~A precision-only
metric is blind to the entire swing -- faithfulness barely moves while recall nearly
doubles for some models. (ii)~The precision/recall trade-off is explicit: stating more
costs precision. (iii)~Even when explicitly told to cover everything, most models still
omit roughly half the relevant facts (recall well below~$1$). (iv)~Prompt response is
strongly model-dependent: gemini-2.5-pro, the abstainer, abstains \emph{harder} under the
cover-all prompt (recall $\AblGeminiRecallA{}\!\to\!\AblGeminiRecallB{}$). A
single-axis faithfulness score reports none of this; precision and recall together do.
...
season (2022 omitted: timing data unavailable). Our headline model comparison focuses on the three core decision types (strategy,
